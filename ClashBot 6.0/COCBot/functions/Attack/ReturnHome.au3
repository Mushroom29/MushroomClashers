...
;		EndIf
    ...
    ; when take loot snapshots is unchecked = 4, when take loot snapshots is checked = 1
    if (GUICtrlRead($chkTakeLootSS) = 4) Then
	   $TakeLootSnapShot = 0
	Else
	   $TakeLootSnapShot = 1
	EndIf
    ...
    ; capture the screen for determining the resource and possibling saving a screenshot
    _CaptureRegion()
    $Raid = 1

	; only save the picture if the option is enabled
